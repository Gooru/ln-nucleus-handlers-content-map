...
public class AJEntityCollection extends Model {
    public static final String SELECT_CUL_COLLECTION_TO_VALIDATE =
        "SELECT id FROM collection where id = ?::uuid AND lesson_id = ?::uuid AND unit_id = ?::uuid AND course_id = ?::uuid AND is_deleted = false";
   
    public static final String SELECT_COLLECTION_TO_VALIDATE =
        "SELECT id FROM collection where id = ?::uuid AND is_deleted = false";
